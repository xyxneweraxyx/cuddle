...
static int alloc_string(dataframe_t *old_df,
    dataframe_t *new_df, int row, int col)
{
    data_t *old_data = &(old_df->columns[col].data[old_df->nb_rows -
            new_df->nb_rows + row]);
    ...
    dataframe_t *new_df, int row, int col)
{
    ...
    data_t *old_data = &(old_df->
        columns[col].data[old_df->nb_rows - new_df->nb_rows + row]);
    data_t *new_data = &(new_df->columns[col].data[row]);
    ...
    if (type == STRING && alloc_string(old_df, new_df, row, col) == DF_FAIL)
        return DF_FAIL;
    return DF_SUCC;
}

static int copy_n_last(dataframe_t *old_df,
    dataframe_t *new_df, int nb_rows)
{
    ...
    for (int i = 0; i < new_df->nb_columns; i++) {
        ...
        for (int j = 0; j < new_df->nb_rows; j++)
            failed = (write_single_data(old_df,
                    new_df, j, i) == DF_FAIL) ? true : failed;
        if (failed)
            break;
    }
...
dataframe_t *df_tail(dataframe_t *dataframe, int nb_rows)
{
    ...
    if (copy_n_last(dataframe, new_df, nb_rows) == DF_FAIL)
        return free_return(false, new_df, new_alloc);
    return new_df;
}
