...
static dataframe_t *free_return(bool success,
    dataframe_t *dataframe, c_alloc_t *buff_alloc)
{
    if (success) {
        if (buff_alloc)
            c_delete(buff_alloc, true);
        return dataframe;
    }
    if (buff_alloc)
        c_delete(buff_alloc, true);
...
static int validate_line(dataframe_t *dataframe, char **values)
{
    int i = 0;

    for (; values[i]; i++);
    if (i != dataframe->nb_columns)
        return DF_FAIL;
    return DF_SUCC;
}
...
static void print_data(dataframe_t *dataframe)
{
    printf("\nCOLUMN NAMES ----\n");
...
char *load_file(const char *filepath, int *size, c_alloc_t *alloc)
{
