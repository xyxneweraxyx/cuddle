...
static void *get_filter_ptr(data_t *data, column_type_t type)
{
    if (type == STRING)
        return (void *)data->str;
    return (void *)data;
}
    ...
    int col, bool (*filter_func)(void *))
{
    ...
    for (int i = 0; i < dataframe->nb_rows; i++)
        if (filter_func(get_filter_ptr(&dataframe->columns[col].data[i],
    ...
    int col, bool (*filter_func)(void *))
{
    ...
    for (int i = 0; i < old_df->nb_rows; i++) {
        if (!filter_func(get_filter_ptr(&old_df->columns[col].data[i], type)))
            continue;
