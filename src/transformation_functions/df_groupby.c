...
static int agg_alloc(groupby_t *gb, const char **to_agg)
{
    int nb_agg = 0;
...
static int setup_result(groupby_t *gb, const char **to_agg)
{
    ...
    if (agg_alloc(gb, to_agg) == DF_FAIL)
        return DF_FAIL;
...
static int fill_group_col(groupby_t *gb)
{
    ...
    for (int i = 0; i < gb->old_df->nb_rows; i++) {
        if (!is_first_occ(gb->old_df, gb->group_col, i))
            continue;
        src = gb->old_df->columns[gb->group_col].data[i];
        dest += 1;
        if (type != STRING) {
            gb->new_df->columns[0].data[dest - 1] = src;
            continue;
        }
        gb->new_df->columns[0].data[dest - 1].str = c_alloc(sizeof(char),
            str_len(src.str) + 1, gb->new_df->alloc);
        if (!gb->new_df->columns[0].data[dest - 1].str)
            return DF_FAIL;
        str_cpy(src.str, gb->new_df->columns[0].data[dest - 1].str);
    }
    return DF_SUCC;
}
