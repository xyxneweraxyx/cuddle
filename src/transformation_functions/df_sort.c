...
static void *get_sort_ptr(dataframe_t *dataframe, int col, int row)
{
    if (dataframe->columns[col].type == STRING)
        return (void *)dataframe->columns[col].data[row].str;
    return (void *)&dataframe->columns[col].data[row];
}
...
static void bubble_sort_iter(dataframe_t *dataframe,
    int col, int i, bool (*sort_func)(void *, void *))
{
    int n = dataframe->nb_rows;

    for (int j = 0; j < n - i - 1; j++) {
        if (sort_func(get_sort_ptr(dataframe, col, j),
                get_sort_ptr(dataframe, col, j + 1)))
    ...
    int col, bool (*sort_func)(void *, void *))
{
    ...
    for (int i = 0; i < n - 1; i++)
        bubble_sort_iter(dataframe, col, i, sort_func);
}
