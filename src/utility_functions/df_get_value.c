...
void *return_val(dataframe_t *dataframe, int i, int row)
{
    if (dataframe->columns[i].type == STRING)
        return (void *)dataframe->columns[i].data[row].str;
...
void *df_get_value(dataframe_t *dataframe, int row, const char *column)
{
    ...
    for (int i = 0; i < dataframe->nb_columns; i++) {
        if (!str_cmp(column, dataframe->columns[i].name))
            return return_val(dataframe, i, row);
    }
    return NULL;
}
